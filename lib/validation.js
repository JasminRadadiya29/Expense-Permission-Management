...
export const validateResetPasswordPayload = (body) => {
  ...
  if (!isNonEmptyString(body?.token) || body.token.trim().length < 20) {
    details.push({ field: 'token', message: 'Reset token is invalid' });
  }
  ...
  if (!isNonEmptyString(body?.confirmPassword) || body.confirmPassword !== body.newPassword) {
    details.push({ field: 'confirmPassword', message: 'Passwords do not match' });
  }
